...
(** Some operations on sum types, that can be used to build plugs  *)
...
Section sum.
    ...
    Definition sum_comm : A + B -> B + A := fun x =>
        match x with
        | inl e => inr e
        | inr e => inl e
        end.
...
Module Stream.
    Section ops.
        ...
        Definition map : stream A -> stream B := fun s t => f (s t).

        Definition zip : stream A -> stream B -> stream (A * B) := fun sA sB t => (sA t, sB t).
...
(** curry/uncurry *)
Definition uncurry {A B C : Type} (f : A * B -> C) : A -> B -> C := fun x => fun y => f (x, y).

Definition curry {A B C : Type} (f : A -> B -> C) : A * B -> C := fun p => f (fst p) (snd p).
