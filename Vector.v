...
Section t.
    ...
    Definition cons {n} x (v : vector n) : vector (S n) := (x, v).
End t.
...
Section eq.
    ...
    Fixpoint veqb n  : vector A n -> vector A n -> bool :=
        match n with
        | 0   => fun l l' => true
        | S m => fun l l' => match l, l' with
                             | (x, q), (x', q') => andb (eq x x') (veqb m q q')
                             end
        end.
End eq.
...
Section map.
    ...
    Fixpoint map n : vector A n -> vector B n :=
        match n with
        | 0   => fun _ => tt
...
Section ops.
    ...
    Fixpoint last n x : vector A n -> A :=
        match n with
        | 0   => fun _ => x
        | S p => fun v => match v with | (t, q) => last p t q end
        end.

    Definition forget_right n (v : vector (A * B) n) : vector A n := map (@fst A B) n v.

    Definition forget_left n (v : vector (A * B) n) : vector B n := map (@snd A B) n v.

    Definition unzip n (v : vector (A * B) n) : vector A n * vector B n :=
        (forget_right n v , forget_left n v).

    Fixpoint zip n : vector A n -> vector B n -> vector (A * B) n :=
        match n with
        | 0   => fun _ _  => tt
        | S p => fun v v' => match v, v' with (t, q), (t', q') => ((t, t'), (zip p q q')) end
        end.

    Fixpoint append n m : vector A n ->  vector A m -> vector A (n + m) :=
        match n with
        | 0   => fun u v => v
        | S p => fun u v => match u with (t, q) => (t, append p m q v) end
        end.

    Fixpoint firstn n m : vector A (n + m) -> vector A n :=
        match n with
        | 0   => fun v => tt
        | S p => fun v => match v with (t, q) => (t, firstn p m q) end
        end.

    Fixpoint skipn n m : vector A (n + m) -> vector A m :=
        match n with
        | 0   => fun v => v
        | S p => fun v => match v with (t, q) => (skipn p m q) end
        end.
End ops.
...
Section pointwise.
    ...
    Fixpoint pointwise n : vector A n -> vector B n -> Prop :=
        match n with
        | 0   => fun _ _  => True
        | S p => fun u    => fun v => match u,v with
